...
const getColor = (wsState, raid) => {
	if (wsState !== "connected") return "darkred";
...
const StatusLine = () => {
	...
	return (
		<StatusLineStyle color={getColor(wsState, inRaid)}>
			<p>
				Status: {wsState === "connected" ? "Connected" : "disconnected"}{" "}
				{user ? `- ${user.player_name} (${user.class_name})` : ""}
			</p>
			<p>
				{inRaid === 1 ? "In Raid" : inRaid === 2 ? "In Dungeon" : "Not in Raid"}
			</p>
		</StatusLineStyle>
	);
};
